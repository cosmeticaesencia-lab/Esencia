...
export type TimelineMilestoneHombres = {
  id?: string;
  label: string;
  image: string;
  imageAlt?: string;
...
type ResultsTimelineHombresProps = {
  milestones: TimelineMilestoneHombres[];
  badge?: string;
  title?: string;
  subtitle?: string;
};
  ...
  return (
    <div
      className={`relative z-10 flex h-7 w-7 items-center justify-center rounded-full border-2 transition-colors duration-300 md:h-8 md:w-8 ${
        active
          ? "border-[var(--h-primary)] bg-[var(--h-primary)] text-white"
...
type TimelineCardProps = {
  milestone: TimelineMilestoneHombres;
  index: number;
};
  ...
  return (
    ...
    >
      <div className="flex items-center gap-2 border-b border-gray-100 px-4 py-3 md:px-5">
        <Calendar className="h-4 w-4 text-[var(--h-primary-dark)]" strokeWidth={2} />
      ...
      <div className="space-y-2 px-4 py-4 md:space-y-2.5 md:px-5 md:py-5">
        <p className="text-[11px] font-semibold uppercase tracking-widest text-[var(--h-primary-dark)] md:text-xs">
          {milestone.eyebrow}
        </p>
...
export default function ResultsTimelineHombres({
  milestones,
  ...
  return (
    ...
    >
      <header className="mx-auto mb-10 max-w-2xl text-center md:mb-14">
        <span className="inline-block rounded-full bg-[var(--h-primary)]/15 px-3 py-1 text-[11px] font-bold tracking-[0.18em] text-[var(--h-primary-dark)] md:text-xs">
          {badge}
        </span>
      ...
      <div ref={timelineRef} className="relative mx-auto max-w-5xl">
        ...
        >
          <motion.div
            className="absolute left-0 top-0 w-full origin-top bg-[var(--h-primary)]"
